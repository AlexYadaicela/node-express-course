...
//   - Should remove Carlos because he has a middle-name
...
const onlyLastNames = names.map((name) => {
  const sName = name.split(" ");
  return sName[sName.length - 1];
});

console.log("Only Last Names", onlyLastNames);

const cFormat = names.filter((name) => {
  return name.match(/^\w+ \w+$/);
});

console.log("Only Correct Format", cFormat);

const covertToTitleCase = names.map((name) => {
  const lName = name.toLowerCase().split(" ");

  lName.forEach((l, index) => {
    if (typeof l[0] === "string") {
      lName[index] = l[0].toUpperCase().concat(l.substring(1));
    }
  });
  return lName;
});

console.log("Covert To Title Case", covertToTitleCase);
...
const result = names
  ...
  .filter((name) => name.slice(-1) !== "z")
  .map((name) => `${name}, Sign up for more information`);
